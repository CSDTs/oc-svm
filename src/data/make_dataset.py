...
def makeinterim(overwrite_any_existing, input_filepath, output_filepath):
    """
    We download the .zip to raw, if overwrite_any_existing = True,
    then we re-save as hdf for much faster I/O.
    """
    logger.info('... filling in data if required...')
    ...
    #  in ./data/raw
    numerai_data = input_filepath/\
        Path(
            os.environ.get('RAW_DATASET_NAME',
                           'numerai_dataset.zip')
        )
    #  in ./data/interim
    ...
    numerai_data_exists = \
        hdf_numerai_data.exists() or numerai_data.exists()
